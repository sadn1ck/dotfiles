...
function prompt_command() {
  PS1="\n${icon_start}$(virtualenv_prompt)${icon_user}${bold_red}\u${normal}${icon_host}${bold_cyan}\h${normal}${icon_directory}${bold_purple}\W${normal}  \$([[ -n \$(git branch 2> /dev/null) ]] && echo \" on ${icon_branch}${space}\")${white}$(scm_prompt_info)${normal}\n${icon_end}"
  PS2="${icon_end}"
}
